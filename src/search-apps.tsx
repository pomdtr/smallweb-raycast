...
export default function SearchApps() {
    ...
    return <List isLoading={isLoading}>{
        apps?.map(app => <List.Item icon={getFavicon(app.url, {
            ...
            fallback: Icon.Globe,
        })} key={app.name} keywords={[app.name]} title={app.name} accessories={[{ text: app.url }]} actions={
            <ActionPanel>
                <ActionPanel.Section>
                ...
                <ActionPanel.Section>
                    <Action.Open shortcut={Keyboard.Shortcut.Common.Open} application="Finder" title="Open in finder" target={app.dir} />
                    <Action.OpenWith shortcut={Keyboard.Shortcut.Common.OpenWith} path={app.dir} />
                    <Action.CopyToClipboard shortcut={Keyboard.Shortcut.Common.CopyPath} title="Copy Path" content={app.dir} />
                </ActionPanel.Section>
            </ActionPanel>
        } />)
    }</List>
